---
title: KubernetesHPA自动扩缩容
description: 本文深入讲解KubernetesHPA自动扩缩容，包含HPA配置、指标采集和扩缩容策略，附带3个可运行代码示例。
date: '2026-05-30'
slug: kubernetes-hpa
tags:
  - 技术
...
通过本文，你将深入理解KubernetesHPA自动扩缩容的核心原理和实践应用，掌握以下关键技能：
...
KubernetesHPA自动扩缩容是DevOps领域的关键技术，掌握这些知识可以实现自动化部署和运维。

### 基础用法

```dockerfile
# KubernetesHPA自动扩缩容基础示例 - Dockerfile
FROM node:18-alpine
WORKDIR /app
COPY package*.json ./
...
```yaml
# KubernetesHPA自动扩缩容进阶示例 - docker-compose.yml
version: '3.8'
...
```yaml
# KubernetesHPA自动扩缩容实战示例 - Kubernetes Deployment
apiVersion: apps/v1
kind: Deployment
metadata:
  name: my-app
