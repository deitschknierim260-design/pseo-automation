---
title: Nginx反向代理
description: 本文深入讲解Nginx反向代理，包含反向代理配置、负载均衡和缓存，附带3个可运行代码示例。
date: '2026-05-30'
...
通过本文，你将深入理解Nginx反向代理的核心原理和实践应用，掌握以下关键技能：
...
Nginx反向代理是DevOps领域的关键技术，掌握这些知识可以实现自动化部署和运维。

### 基础用法

```dockerfile
# Nginx反向代理基础示例 - Dockerfile
FROM node:18-alpine
WORKDIR /app
COPY package*.json ./
...
```yaml
# Nginx反向代理进阶示例 - docker-compose.yml
version: '3.8'
...
```yaml
# Nginx反向代理实战示例 - Kubernetes Deployment
apiVersion: apps/v1
kind: Deployment
metadata:
  name: my-app
