---
title: Python生成器和迭代器
description: 本文深入讲解Python生成器和迭代器，包含yield关键字、迭代器协议和生成器表达式，附带3个可运行代码示例。
date: '2026-05-30'
...
Python生成器和迭代器是Python编程中的核心进阶技能，涉及协程、事件循环、异步I/O等关键概念。掌握这些技术能够显著提升程序的并发性能，特别适用于I/O密集型应用场景。
...
yield关键字是Python 3.5引入的异步编程语法，通过`async`和`await`关键字实现协程。协程是一种轻量级的并发编程方式，可以在单线程内实现多个任务的并发执行。
...
生成器表达式涵盖了异步编程在实际项目中的应用，包括网络爬虫、API客户端、实时数据处理等场景。

#### 2. 完整实战示例

```python
# Python生成器和迭代器实战示例 - 异步爬虫
import asyncio
import aiohttp
...
1. **yield关键字**: 掌握async/await语法，理解协程基本概念
2. **迭代器协议**: 深入理解事件循环机制，掌握并发控制策略
3. **生成器表达式**: 实践异步爬虫、API客户端等真实场景
